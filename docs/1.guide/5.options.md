...
HTTPS configuration options. When provided, the server will use HTTPS instead of HTTP.
...
// Or using certificate content directly
serve({
  https: {
    inlineKey: process.env.SSL_KEY,
    inlineCert: process.env.SSL_CERT,
    // Optional: CA certificate for self-signed or private CAs
    inlineCa: [process.env.SSL_CA],
    // Optional: Private key passphrase if encrypted
